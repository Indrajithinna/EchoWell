-- Fixed Daily Conversation Tracking Schema
...
-- Voice tone records
CREATE TABLE IF NOT EXISTS voice_tone_logs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id TEXT REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_voice_tone_user ON voice_tone_logs(user_id, recorded_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_conversation_metrics_user ON conversation_metrics(user_id, created_at DESC);
...
DROP POLICY IF EXISTS "Users can view own voice logs" ON voice_tone_logs;
DROP POLICY IF EXISTS "Users can view own metrics" ON conversation_metrics;
